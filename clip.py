...
def save_to_xml(save_path, im_width, im_height, objects_axis, label_name, name, hbb=True):
    ...
    for i in range(object_num):
        ...
        if label_name[int(objects_axis[i][-1])] not in set:
           set.append(label_name[int(objects_axis[i][-1])])
        ...
        if hbb:
            ...
            if objects_axis[i][5]<0:
                objects_axis[i][5]=0
            if objects_axis[i][0] >split_maxy:
                objects_axis[i][0] = split_maxy-1
            if objects_axis[i][1] > split_maxy:
                objects_axis[i][1] = split_maxy-1
            if objects_axis[i][2] >split_maxy :
                objects_axis[i][2] = split_maxy-1
            if objects_axis[i][5] >split_maxy:
                objects_axis[i][5] = split_maxy-1
...
parser.add_argument('--raw_data', type=str, default='E:/DOTA_datasets', help='dataset from')
parser.add_argument('--save_dir', type=str, default='H:/DOTA/DOTA_clip/train/', help='where clip dataset save ')
parser.add_argument('--split_maxx', type=int, default=1024,)
parser.add_argument('--split_maxy', type=int, default=1024,)
parser.add_argument('--stride', type=int, default=800,)


opt = vars(parser.parse_args())
print('class_list', len(class_list))
raw_data = opt['raw_data']
...
split_maxx=opt['split_maxx']
split_maxy=opt['split_maxy']
stride=opt['stride']
pr=tqdm.tqdm(images)
for idx, img in enumerate(pr):
    # img = 'P1524.png'
    # print(idx, 'read image', img,flush=True)
    pr.set_description(str(idx)+'read image'+img)
    ...
    clip_image(img.strip('.png'), img_data, box, split_maxx, split_maxy)
